...
class Task(Base):
    ...
    rejected_by_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("users.id"), nullable=True)
    rejected_by_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    rejection_reason: Mapped[str | None] = mapped_column(String(255), nullable=True)
    rejection_comments: Mapped[str | None] = mapped_column(Text, nullable=True)
